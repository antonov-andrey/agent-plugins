...
`linear-agent-tools:workflow-configure` явно подготавливает exact Linear destination и explicit GitHub repository/base: team-level issue statuses в fixed categories, workspace-level Project statuses `Planned`, `In Progress`, `Completed`, `Canceled`, role labels, required `agent:codex` label, полное отсутствие team Git status automations и merge-mechanism-compatible branch protection. GitHub integration остаётся включённой только как официальный issue-to-branch/PR link owner; PR events не меняют task status, потому что lifecycle transitions, evidence gates и cleanup принадлежат provider workflow. GitHub protection read связывает repository/base с authenticated login и permission, classic protection, fully paginated active effective rules, full ruleset identities, bypass actors и required-check definitions; missing, disabled, ineffective, bypassed, malformed или failed provider state отвергается. Для absent protection approved exact path может создать только minimal classic no-bypass protection без human PR gate и required checks для two-ref atomic `merge`; existing protection не ослабляется. `squash`/`rebase` требуют existing strict up-to-date protection и хотя бы один required check. `workflow-configure` показывает exact global delta, применяет только утверждённые identities и требует owner read-back. Identity-preserving status updates мигрируют exact provider-owned `Human Review` в `Review` и удаляют human-approval wording из exact legacy `Merging` definition через native `workflowStateUpdate` с теми же Linear IDs; foreign, duplicate либо одновременно legacy/current review status останавливает mutation. Он не создаёт task graph и не меняет Product source. Official Linear MCP и user-level OAuth используются первыми; missing provider operation доказывается contract probe до появления minimal Linear-specific GraphQL boundary.
