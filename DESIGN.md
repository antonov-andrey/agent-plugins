...
Каждая top-level agent attempt до dispatch или status mutation получает один process-lifetime host-local lock по exact Linear issue и удерживает его до attempt-resource cleanup и final Linear read-back; crash освобождает kernel lock. Namespace lock выводится только из explicit canonical multi-repository `LINEAR_AGENT_WORKSPACE_ROOT`, issue и purpose; workspace root не может быть Git repository/worktree, а CWD, repository и task worktree не меняют identity. Nested cleanup переиспользует guard вызывающей attempt, а отдельный short-lived operation lock сериализует только Git workspace и cleanup transactions. Каждая code-mutating issue использует branch `linear/<lowercase-issue-identifier>` и project-local `.worktree/<lowercase-issue-identifier>`. `linear-agent-tools:task-implement` владеет creation, adoption, bootstrap, crash recovery и fresh-thread implementation attempts. Первый prepare фиксирует exact remote-base baseline; `Rework` принимает existing branch/PR без destructive reset. `task-review` независимо выводит complete coverage и не меняет Product code. Candidate-review findings возвращают implementation в `Rework`; findings против merged state принадлежат review/acceptance no-fix boundary, а graph mutation выполняет `task-graph-create` как approved active-Project delta. `task-merge` работает только с direct PR URL/base-branch/base-commit/head map из zero-finding review и same-repository head и отвергает effective merge-queue rules как deferred mutation против later base. Для open PR absent auto-merge request, closed protection/check snapshot и exact executing principal проверяются до mutation. Declared `merge` разрешает каждый effective origin fetch/push URL и applicable `url.*.insteadOf`/`pushInsteadOf`, отвергает multiple, divergent, noncanonical, rewritten и foreign destinations, затем выполняет push и immediate immutable readback через один explicit canonical GitHub HTTPS URL. Invocation-local credential helper получает token внутри Git subprocess через exact named `gh` account; token не входит в Python process/argv/log и persistent credential config не меняется. Один server-atomic protected-ref CAS transaction одновременно проверяет reviewed old OIDs base/head, fast-forward обновляет base exact no-ff merge commit и удаляет head; любой race отклоняет оба updates. CAS требует exact zero required-check definitions, потому что новый merge commit не может получить provider checks до atomic push. `squash`/`rebase` используют synchronous provider merge с exact head match только под strict up-to-date protection и хотя бы одним required check. Typed valid zero set является CAS-only; generic exit `1`, empty/malformed output, protection absence/disable/bypass не превращаются в success. Already-merged recovery сначала читает terminal PR/result и не зависит от later protection/check definitions, mutable title либо advanced current base tip; acceptance требует immutable reviewed base/head, exact merge commit tree и ordered parents, provider terminal identity, canonical destination и exact deleted reviewed head. Любой lease rejection либо changed base branch/base commit/head до successful mutation требует nested cleanup и `Rework` без fix-forward. Per-PR human approval workflow отсутствует. Каждый graph завершается exact cleanup, а Product commit/push остаются у `agent-workflows:git-commit`.
...
Linear status history владеет wall-clock lifecycle. Каждая attempt boundary сначала завершает nested attempt-resource cleanup, затем добавляет один provider-marked compact semantic handoff с cleanup proof, operation, role/delivery, outcome, UTC interval, concise current state, delivery-applicable commits, direct PR URL/base-branch/base-commit/head map, verification summaries и bounded evidence URLs. Handoff служит context для semantic recovery, а не approval object или automatic cache. Непустое closed subset доступных exact structured `input_tokens`, `cached_input_tokens`, `cache_write_input_tokens`, `output_tokens`, `reasoning_output_tokens` агрегируется и валидируется независимо; usage object отсутствует только когда counters не exposed, а unknown, invalid, empty, estimated usage и derived totals запрещены. Evidence-only, review, acceptance и cleanup не заявляют changed Product commits. До final-boundary handoff и `Review` complete local acceptance baseline с ровно queue, startup, execution, review и merge durations из exact provider history публикуется, byte-read и включается evidence URL в handoff. Эти objects строит shared `verification` owner; local telemetry database не требуется.
