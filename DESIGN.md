...
`linear_boundary` разделяет shared validation и status vocabulary, один `configuration/` owner для workspace-global configuration, один `task/` owner для dispatch и transitions и отдельный HTTP transport. Configuration GraphQL adapter не владеет task state machine, task workflow не владеет authenticated workspace configuration, а transport не знает Product policy. Package `__init__.py` не является re-export facade: потребитель импортирует символ из его defining module, поэтому перенос владельца не маскируется compatibility alias.
