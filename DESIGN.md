...
`agent-workflows:goal-brainstorm` владеет общей подготовкой изолированного task worktree для каждого persistent implementation goal. Его stable contract состоит из skill entrypoint, `plugins/agent-workflows/skills/goal-brainstorm/references/specification-contract.md` и `plugins/agent-workflows/skills/goal-brainstorm/references/worktree-contract.md`.

Физическая harness-neutral пара task artifacts остаётся в `.spec/` coordinating main worktree. Specification, goal, task branch и linked-worktree directory используют один точный общий префикс; project-local linked worktrees размещаются под единственным корнем `.worktree/`. Git-internal `.git/worktrees/` не является альтернативным project path.

Каждый участвующий top-level repository и каждый task-owned submodule обязаны иметь tracked root manifest `worktree-bootstrap.toml`. Простой read-only checkout submodule на recorded gitlink не получает manifest change только из-за инициализации. Provider contract определяет versioned schema и общую семантику resource classes; конкретный project `Key Directory Map` связывает manifest path с `agent-workflows:goal-brainstorm`, а сам manifest содержит только project-owned exact paths. Отсутствующий manifest создаётся в task worktree до реализации текущего goal и не запускает не объявленную specification workspace-wide migration остальных repositories.

Переиспользуемая реализация принадлежит библиотечному модулю `plugins/agent-workflows/lib/goal-brainstorm/worktree.py`. Тонкий исполняемый интерфейс принадлежит skill-local script `plugins/agent-workflows/skills/goal-brainstorm/scripts/worktree.py`; отдельный `lib/goal-brainstorm/tool/` не существует. Команда `prepare` создаёт task worktrees, инициализирует recursive submodules, явно классифицирует изменяемые submodule boundaries и применяет manifests; `contracts-authored` после complete worktree validation явно фиксирует завершение stable-owner authoring и закрывает repository/submodule set перед semantic review; `validate` диагностирует drift, выполняет безопасные однозначные repairs и повторяет полную проверку; `recover-main-leak` восстанавливает recorded main preimage только для exact paths, происхождение которых явно подтверждено вызывающим агентом и повторно доказано совпадающими task/main fingerprints; `accept-main-commit-drift` после явного пользовательского решения фиксирует exact full current commit и полный reported overlap path set как независимую committed main work на exact top-level или task-owned-submodule owner boundary, не позволяет родителю аттестовать delegated submodule path, сворачивает recorded dirty-to-committed переход только при точном совпадении committed object с captured working preimage, не изменяет ни main, ни task checkout и не распространяет решение на последующий drift; `seal` только из `contracts_authored` или inactive `goal_ready` фиксирует immutable task-artifact inputs перед activation; `activate` после успешного harness-owned создания persistent goal повторно проверяет sealed state и записывает `active` без harness-specific goal logic. Все команды принимают явные repository roots и task artifact identity и не содержат списка workspace projects или application-specific resource paths.

Private preparation state хранится только в Git administration space participating repositories. Он связывает task identity с exact main and task roots, baseline commits and dirty-state fingerprints, caller-attested exact committed main overlaps, physical non-symlink submodule roots, nested main preimages, manifest results, artifact hashes и временными local excludes. До создания branch/worktree provider записывает pending ownership, а resource, main-leak и ordinary-text mutations выполняет через durable transactions. Private-file writes и clone-based preimage или copy-fingerprint-migration operations используют unpredictable staging identity с предварительно атомарно опубликованным intent, поэтому partial object после process death имеет доказанного владельца, а deterministic filename сам по себе владельца не доказывает. Collision-safe fingerprints включают type, mode, structure, raw link target и length-delimited bytes; filesystem text сохраняет non-UTF-8 names через surrogate escape. Transaction ownership и obsolete resource preimages удаляются только после записи полного state во все replicas; каждый automatic recovery отражается в command result. `.spec/` не содержит harness state.

Tracked task changes создаются и проверяются только в записанных task roots. Pending или markerless adoption доказывает exact baseline, index, branch, registered path, Git administration identity и допустимые provider outputs до project writes. Wrong-root invocation автоматически перенаправляется в записанный worktree, когда это однозначно; повреждение worktree metadata, link, clean submodule или provider-created resource сначала диагностируется и безопасно исправляется. Возможная пользовательская работа не перезаписывается без явного решения.

Worktree tool не commit, push, merge и не удаляет worktree. После отдельного явного publication request эти операции принадлежат `agent-workflows:git-commit`; до доказанного merge task-owned tracked changes в main worktree запрещены.
