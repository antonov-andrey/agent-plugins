...
Verification receipt хранится в Linear issue или GitHub check и допускает reuse только для exact source fingerprint, semantic verification-contract fingerprint, direct command, canonical absolute working directory, corpus content, model, model configuration и применимых environment или release identity. Verification-contract fingerprint вычисляется из фактических assertions, expected behavior, output schema и invariants конкретной проверки; issue, delta, attempt, Codex version и другие orchestration identities не включаются, если они не меняют семантику результата. Checkout list хранит для каждого result-affecting checkout его canonical absolute path, roles, credential-free supported repository URL, full commit, recursive submodule commits и dependency locks с canonical checkout-relative paths, поэтому две ревизии одного repository не коллидируют. Stable verification key связывает только полный набор result-affecting inputs. Отдельный receipt key связывает verification key, outcome, UTC completion instant, canonical artifact URL и content SHA-256. Artifact URL обязан быть durable canonical HTTPS provider identity без credentials, port, query или fragment, поэтому receipt не сохраняет expiring presigned URL; из numeric hosts поддерживается только canonical dotted-decimal IPv4, а legacy, hex и out-of-range all-numeric forms не являются canonical provider hosts. Shared verification tool получает exact result-completion instant, создаёт codec-rendered receipt comment и проверяет reuse по exact provider readback; каждый workflow role публикует, полностью перечитывает и byte-сравнивает этот comment до удаления transient input или candidate publication. Receipt comment кодирует JSON forward slashes как escapes: после JSON parsing значение URL остается точным, но Linear readback не заменяет canonical identity presigned autolink. Поддерживается только одна current schema без legacy compatibility branch. Изменение любого declared input инвалидирует только зависимые receipts, а изменение result или evidence identity требует нового coherently issued receipt. Exact candidate fingerprint строится из sorted PR URL/head pairs для code delivery либо из compact sorted map evidence kind -> receipt key для evidence delivery. Evidence candidate input содержит complete passed current-schema receipts: shared tool сначала проверяет derived receipt key каждого receipt, поэтому failed receipt или stable verification key нельзя подставить как receipt-bearing approval identity, а изменение outcome, completion instant, canonical artifact URL или artifact SHA инвалидирует candidate. Compact candidate identity и derived fingerprint сохраняются вместе в typed attempt comment; provider readback заново выводит fingerprint из identity и поэтому достаточен для fresh merge/review без transient preimage. Смешанный или неполный candidate запрещён. Targeted checks выполняются по owner slices, full checks — на frozen candidate, а fresh terminal semantic audit — после последнего fix. Raw logs не переносятся в model context без необходимости.

Linear status history владеет wall-clock lifecycle. Каждая agent attempt добавляет один provider-marked typed concise telemetry summary с role, delivery kind, outcome, UTC interval, delivery-applicable commit set, receipt hit/miss, external wait, compact candidate identity и её exact derived fingerprint когда candidate создан, а также доступным token usage без prompt bodies, secrets или raw logs. Evidence-only, review, acceptance и cleanup attempts не могут заявлять Product commits. Complete local acceptance baseline содержит ровно queue, startup, execution, review и merge durations, выведенные из exact provider history. Эти evidence objects детерминированно строит shared `verification` owner; отдельная local telemetry database не является prerequisite workflow.
