...
`task_cleanup/reconciliation.py` владеет одним bounded cleanup workflow. Его public `cleanup` удерживает issue lock и явно упорядочивает repository-state read-back, terminal identity snapshot, PR reconciliation, declared-resource cleanup, workspace removal и Project-final absence proof. Общие изменяемые counters и repository snapshots принадлежат одному run-local `CleanupState`; immutable config и injected GitHub/resource boundaries остаются на workflow owner. Добавление нового cleanup boundary обязано либо переиспользовать существующего injected collaborator, либо создать cohesive collaborator в `task_cleanup`, а не возвращать boundary implementation в orchestration method, создавать hidden dependency или передавать один и тот же широкий набор несвязанных аргументов между generic helpers.
