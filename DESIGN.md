...
3. Не более одного detached-index commit меняет только два объявленных root contract path. Non-force compare-and-swap push публикует его, canonical checkout fast-forward-ится, а exact commit и diff перечитываются.
...
Между direct-main publication и успешным fresh discovery `project-goals` fail closed на отсутствующем required skill. Единственное pre-install исключение — exact existing issue `AND-47` со stable task key `project-goals-linear-project-sync-implementation`, когда fresh guarded attempt началась из `Rework` из-за confirmed independent Review finding для этого же cutover. Latest byte-read Review handoff обязан называть exact open `agent-plugins` candidate и exact reviewed direct-main commit. Под live `AND-47` attempt guard и тем же workspace-global direct-main lock эта attempt может исправить только `project-goals/AGENTS.md` и `project-goals/DESIGN.md`, и только в части confirmed finding. Сначала она исправляет, проверяет и push-ит `agent-plugins` candidate, затем как последнюю pre-Review mutation публикует не более одного двухфайлового correction commit либо доказывает semantic no-op. Ordinary `project-goals` mutation остаётся disabled. Это исключение не разрешает `goal-brainstorm`, новый skill, task, branch, worktree, pull request, source-file edit или иной compatibility route.

Expected direct-main base для Rework является exact commit из finding handoff. До correction push `origin/main` обязан равняться этому commit. Crash retry может принять один уже опубликованный successor только когда его parent является expected base, его diff ограничен двумя authorized paths, а fresh semantic reread подтверждает exact correction того же finding против current PR candidate. После такого recovery canonical checkout fast-forward-ится и exact commit/diff перечитываются без повторной mutation. Любой другой remote state является drift и останавливает attempt. Новый Review заново связывает recovered либо newly published commit с current PR candidate. Если merge уже завершён, installation, discovery, migration или activation failure не откатывает published repositories: `AND-47` остаётся в `Merging`, а guarded recovery продолжает с semantic current state. Alias, forwarding skill, journal, receipt, fingerprint и dual compatibility wording запрещены.
