...
Task identity является парой exact Project UUID и stable task key, представление которого задаёт canonical card contract. Title, card content, source commit, Linear issue number и content hash не входят в identity. Zero key matches создаёт issue, one match сохраняет exact issue UUID, comments, attachments и history, а duplicate key останавливает synchronization. Linear statuses, role и dispatch labels, assignee либо delegate, comments, attachments и native blocker relations являются единственным operational graph state. Git и GitHub остаются владельцами branch, commit, PR, check, review и merge identities.
...
| `Completed` или `Canceled` | Полностью совпадающая synchronized revision является no-op. Для новой approved revision перевести тот же Project в non-dispatchable `Planned`, сохранить terminal history, stage новую revision и активировать тот же Project только complete `Planned -> In Progress` transition. |
| Absent, foreign или unsupported status value | Остановить synchronization без mutation. |

Одинаковая approved commit revision с уже совпадающим semantic state является no-op. Одинаковая revision после interruption всё равно reconciles current state и не принимает revision metadata как completion proof. Новая commit revision может не менять task graph; тогда synchronization обновляет revision metadata только после complete readback. Последняя synchronized commit записывается после всех issue activations, а для `Planned` Project — перед final activation. Crash recovery повторно читает полностью paginated Project, issues, labels и blocker relations и продолжает следующую semantic phase без receipt или expected historical prose.

Existing issue с неизменившимся semantic task contract не мутируется. Changed issue в `Backlog` или `Todo` сначала становится non-dispatchable, затем обновляется in place и активируется только после readback. Changed issue в `In Progress`, `Review`, `Rework`, `Merging`, `Done` или `Canceled` не переписывается: approved plan создаёт explicit replacement issue с новым meaningful stable task key, old issue блокирует replacement, а replacement блокирует current downstream Review. Terminal issues никогда не reopen и не меняют delivered history. Удалённая новой revision inactive issue может перейти в `Canceled` только как часть явно утверждённого synchronization plan; removal active issue требует отдельного explicit human cancellation decision, а terminal issue сохраняется без mutation.
...
Один plugin-owned card contract является единственным владельцем visible card shape, optional sections и их omission rules. Stable architecture требует от него только human-readable semantic task contract и stable task key; operational Linear state не дублируется в card. Linear issue является canonical per-task goal и execution journal.
