...
Verification receipt является typed evidence object с отдельными identities для полного result-affecting input, exact result и approval candidate. Полный operational contract, включая immutable external input artifacts, current schema, creation/readback/reuse proof и candidate derivation, имеет единственного owner: `plugins/linear-agent-tools/references/manual-workflow.md`, раздел `Verification Receipt Contract`; role skills ссылаются на него и не копируют field checklist или обязательные фразы. Targeted checks выполняются по owner slices, full checks — на frozen candidate, а fresh terminal semantic audit — после последнего fix. Raw logs не переносятся в model context без необходимости.
