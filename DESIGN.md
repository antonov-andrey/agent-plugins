...
Linear status history владеет wall-clock lifecycle. Каждая attempt boundary добавляет один provider-marked compact semantic handoff с operation, role/delivery, outcome, UTC interval, concise current state, delivery-applicable commits, direct PR URL/head map, verification summaries и bounded evidence URLs. Handoff служит context для semantic recovery, а не approval object или automatic cache. Доступные exact structured `input_tokens`, `cached_input_tokens`, `cache_write_input_tokens`, `output_tokens`, `reasoning_output_tokens` агрегируются независимо; missing usage опускается, estimates и derived totals запрещены. Evidence-only, review, acceptance и cleanup не заявляют changed Product commits. Complete local acceptance baseline содержит ровно queue, startup, execution, review и merge durations из exact provider history. Эти objects строит shared `verification` owner; local telemetry database не требуется.
