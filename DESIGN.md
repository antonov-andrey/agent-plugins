...
Provider-owned Project descriptions, issue descriptions и transaction documents сразу рендерятся в canonical Markdown Linear: списки используют `*`, а absolute HTTP(S) references — explicit links. Reconciler сравнивает именно эту read-after-write форму; он не предполагает, что Linear сохранит произвольный входной Markdown byte-for-byte, и не вводит permissive normalization, которая могла бы скрыть пользовательское изменение provider-owned content.
