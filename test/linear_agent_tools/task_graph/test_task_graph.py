...
def test_graph_supports_exact_delegate_assignment_without_null() -> None:
    ...
    issue = next(item for item in graph_publication_view_build(graph).issue_list if item.node_key == "implementation")
...
def test_cross_repository_code_requires_visible_ordered_partial_merge_recovery() -> None:
    ...
    view = graph_publication_view_build(TaskGraph.from_payload(payload))
...
def test_activation_barrier_advances_one_idempotent_phase_at_a_time() -> None:
    ...
    document_plan = reconciliation_plan_build(graph, _remote(graph, document=False, issues=False))
    assert document_plan.phase is PublicationPhase.DOCUMENT
    assert (
        reconciliation_plan_build(graph, _remote(graph, document=True, issues=False)).phase is PublicationPhase.ISSUES
    )
    issue_creation = reconciliation_plan_build(graph, _remote(graph, document=True, issues=False))
    ...
    assert (
        reconciliation_plan_build(graph, _remote(graph, document=True, issues=True)).phase is PublicationPhase.RELATIONS
    )
    assert (
        reconciliation_plan_build(
            graph,
...
def test_activation_readback_proves_exact_handoff_without_freezing_later_linear_state() -> None:
    ...
    assert reconciliation_plan_build(graph, progressed).phase is PublicationPhase.COMPLETE
...
def test_project_cancellation_stops_dispatch_before_canceling_unfinished_issues() -> None:
    ...
    with pytest.raises(TaskGraphError, match="completed.*cannot be canceled"):
        cancellation_plan_build(graph, replace(active, status_name="Completed"), human_decision=True)
...
def test_partial_or_damaged_import_can_be_canceled_without_an_import_document() -> None:
    ...
    assert (
        cancellation_plan_build(graph, active_without_document, human_decision=True).phase
...
def test_active_project_delta_advances_issue_relation_metadata_and_todo_separately() -> None:
    ...
    assert delta_reconciliation_plan_build(delta, remote).phase is PublicationPhase.DELTA_ISSUES
