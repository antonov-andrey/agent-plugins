...
After every project main is the exact selected snapshot, deploy and verify the full permanent primary environment using the owning repositories' current acceptance rules. A failure leaves the merge journal and accepted pointer unchanged. Fix every in-scope non-destructive failure in the recorded task worktrees, run the affected and complete verification, use `agent-workflows:git-commit`, and publish one complete descendant checkpoint through `agent-workflows:goal-checkpoint`. Resume merge with that checkpoint in the same exclusive workflow. Do not request separate approval for this fix-forward.

Stop and request a decision only when the fix needs authority outside the approved goal or would lose data because no lossless migration exists. Do not use this exception for ordinary code repair, compatible migration, recovery, deployment retry, or acceptance fallout.
