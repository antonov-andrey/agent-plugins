...
- PR creation and merge use the complete exact PR set for `(repository, base, deterministic task branch)`. The review handoff's direct PR URL/base-branch/base-commit/head map is reread against current GitHub state before `Review -> Merging` and again before merge. Every effective merge-queue rule is rejected because queuing or auto-merge would defer mutation to a later base. For an open candidate, declared `merge` resolves all effective fetch/push destinations and URL rewrites, pins one canonical GitHub HTTPS destination plus an invocation-local credential helper to the same freshly reread `gh` principal whose authority was inspected, and applies one server-atomic protected-ref transaction with explicit old-OID leases for both reviewed base and head; a race rejects the base update and head deletion together. This path requires an exact zero required-check definition set because its new merge commit cannot have checks before the atomic push. Declared `squash`/`rebase` instead requires exact head matching plus no-bypass strict up-to-date protection with at least one required check. Required-check definitions come from typed protection and current results come from successful typed provider output; a legitimate zero set is distinct from exit `1`, empty or malformed output and is CAS-only. For an already merged candidate, current protection, checks, title and base tip are not gates: recovery proves the immutable reviewed commits, exact terminal merge tree and ordered parents, provider terminal identity, canonical repository destination and exact deleted head. Any reviewed base branch, base commit, head or result-affecting source change before mutation, including a lease rejection in the preflight-to-mutation window, completes nested cleanup and goes to `Rework`; merge never fixes forward.
