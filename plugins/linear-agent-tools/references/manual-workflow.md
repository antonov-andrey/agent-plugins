...
Every attempt comment binds its role, delivery kind and outcome to the applicable candidate and evidence shape. Evidence-only, review, acceptance and cleanup attempts cannot report changed Product commits; completed code delivery must report its exact changed commit set. `human-review` and `merged` persist both the complete compact candidate identity and its derived fingerprint; provider readback must reproduce that fingerprint from the identity. Remediation and cleanup use no candidate; completed outcomes require bounded evidence links. This comment is telemetry and evidence, not another lifecycle database.
