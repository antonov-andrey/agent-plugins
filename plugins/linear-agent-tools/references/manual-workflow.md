...
4. Every cleanup/recovery authority-bearing Git read and mutation uses the provider's one closed minimal standard-user invocation owner with global/system and invocation Git config ignored, replacement and alternate object resolution disabled, hooks inert, and only the exact destination protocol and credentials enabled before remote deletion or canonical-base fast-forward.
