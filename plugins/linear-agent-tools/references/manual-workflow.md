...
- Git owns branches and commits. GitHub owns PR state, required checks, branch protection and merge results.
...
- `Merging`: merge may operate only on the exact PR heads covered by the zero-finding independent review.
...
- External waits use the harness's native background terminal and native wait/resume. Do not build model polling, a Project supervisor, command timeouts, arbitrary scheduling thresholds or an alternate Codex home around CI, provider or long-running checks.
...
- PR creation and merge use the complete exact PR set for `(repository, base, deterministic task branch)`. The review handoff's direct PR URL/head map is reread against current GitHub state before `Review -> Merging` and again before merge. Any head or result-affecting source change goes to `Rework`; merge never fixes forward.
- Every agent-owned result transition requires the delivery-applicable semantic handoff and direct evidence to be published and reread first, including review findings, remediation, changed reviewed state, merge completion and cleanup completion.
...
Linear and GitHub history own wall-clock lifecycle. When acceptance explicitly owns a local-flow baseline, record exactly queue, startup, execution, review and merge durations from provider history through `evidence.py baseline`; never estimate missing phases or parse private chat logs.
