...
class TaskTransition:
    ...
    def require(self) -> None:
        ...
        if not isinstance(self.proof, TransitionProof) or not isinstance(
            self.dispatchable, bool
        ):
        ...
        if self.target is IssueStatusName.CANCELED and self.current not in {
            ...
            return
        if (
            self.current is IssueStatusName.BACKLOG
            and self.target is IssueStatusName.TODO
        ):
        ...
        if (
            ...
            return
        if (
            self.current is IssueStatusName.IN_PROGRESS
            and self.target is IssueStatusName.HUMAN_REVIEW
        ):
            common_ready = all(
                (
            ...
            if self.delivery_kind == "code":
                common_ready = (
                    common_ready
                    and self.proof.publication_ready
                    and self.proof.required_ci_ready
                )
        ...
        if (
            ...
            return
        if (
            self.current is IssueStatusName.HUMAN_REVIEW
            and self.target is IssueStatusName.REWORK
        ):
        ...
        if (
            self.current is IssueStatusName.HUMAN_REVIEW
            and self.target is IssueStatusName.MERGING
        ):
            if (
                self.delivery_kind != "code"
                or not self.proof.human_decision
                or not self.proof.candidate_unchanged
            ):
                raise LinearContractError(
        ...
        if (
            self.current is IssueStatusName.HUMAN_REVIEW
            and self.target is IssueStatusName.DONE
        ):
            if (
                self.delivery_kind == "code"
                or not self.proof.human_decision
            ...
            return
        if (
            self.current is IssueStatusName.MERGING
            and self.target is IssueStatusName.DONE
        ):
        ...
        if self.current is IssueStatusName.TODO and self.target is IssueStatusName.DONE:
            ...
            return
        raise LinearContractError(
            f"Workflow transition {self.current} -> {self.target} is forbidden"
        )
