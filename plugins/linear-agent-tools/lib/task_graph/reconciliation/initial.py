...
class TaskGraphReconciler:
    ...
    def activation_readback_require(self, remote: RemoteProject) -> ReconciliationPlan:
        ...
        if (
            remote.name != self._view.project_name
            or remote.status_name != "In Progress"
        ):
    ...
    def cancellation_plan(
        self, remote: RemoteProject, *, human_decision: bool
    ) -> ReconciliationPlan:
        ...
        if not isinstance(human_decision, bool) or not human_decision:
            raise TaskGraphError(
                "Project cancellation requires an explicit human decision"
            )
        remote.initial_identity_require(self._view)
    ...
    def plan(self, remote: RemoteProject | None) -> ReconciliationPlan:
        ...
        for desired in self._view.issue_list:
            ...
            for blocker_key in sorted(
                desired_blocker_set - set(current.blocker_key_list)
            ):
        ...
        if relation_action_list:
            return ReconciliationPlan(
                PublicationPhase.RELATIONS, relation_action_list, activation_ready=False
            )
