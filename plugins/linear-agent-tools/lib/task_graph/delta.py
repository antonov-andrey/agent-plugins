...
class DeltaProvenance:
        ...
        for label, value in (
            ...
            if (
                ...
                raise TaskGraphError(
                    f"Delta provenance {label} must be non-empty single-line text"
                )
    ...
    def from_payload(cls, payload: object) -> "DeltaProvenance":
        ...
        try:
            ...
            raise TaskGraphError(
                "Delta provenance contains an unsupported field value"
            ) from error
...
class TaskGraphDelta:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise TaskGraphError(
                "Delta team_id and project_id must be lowercase Linear UUIDs"
            )
        if not isinstance(self.source, SourceIdentity) or not isinstance(
            self.provenance, DeltaProvenance
        ):
        ...
        expected_project_key = (
            f"linear-agent-tools:v1:{self.team_id}:{self.source.fingerprint()}"
        )
        if self.project_key != expected_project_key:
            raise TaskGraphError(
                "Delta Project key differs from its immutable original source"
            )
        ...
        if any(item.role is TaskRole.CLEANUP for item in self.node_list):
            raise TaskGraphError(
                "An active Project delta must reuse its one existing final cleanup node"
            )
        resource_key_list = [
        ...
        if len(resource_key_list) != len(set(resource_key_list)):
            raise TaskGraphError(
                "Graph delta repeats one resource key across new issue owners"
            )
        if (
            ...
            raise TaskGraphError(
                "Graph delta must name a duplicate-free existing node set"
            )
        if any(
            _NODE_KEY_PATTERN.fullmatch(item) is None
            for item in self.existing_node_key_list
        ):
            raise TaskGraphError(
                "Graph delta existing node keys must be lowercase semantic slugs"
            )
        ...
        if (
            ...
            or any(
                not isinstance(item, str) for item in self.reverification_node_key_list
            )
            or len(self.reverification_node_key_list)
            != len(set(self.reverification_node_key_list))
        ):
            raise TaskGraphError(
                "Graph delta reverification node keys must be a duplicate-free list"
            )
        if any(
            _NODE_KEY_PATTERN.fullmatch(item) is None
            for item in self.reverification_node_key_list
        ):
            raise TaskGraphError(
                "Graph delta reverification node keys must be lowercase semantic slugs"
            )
        unknown_reverification_key_set = set(self.reverification_node_key_list) - set(
            self.existing_node_key_list
        )
        ...
        if not isinstance(self.blocker_edge_list, list) or any(
            ...
            raise TaskGraphError(
                "Graph delta blocker edge list must contain only named blocker edges"
            )
        ...
        for edge in self.blocker_edge_list:
            ...
            if not edge_node_key_set <= known_key_set:
                raise TaskGraphError(
                    "Graph delta contains a malformed or unknown blocker edge"
                )
            if not edge_node_key_set & set(new_node_by_key_map):
                raise TaskGraphError(
                    "Graph delta may not mutate relations solely between existing nodes"
                )
        new_node_key_set = set(new_node_by_key_map)
        for node_key in self.reverification_node_key_list:
            if not any(
                edge.blocked_node_key == node_key
                and edge.blocker_node_key in new_node_key_set
                for edge in edge_set
            ):
                raise TaskGraphError(
                    f"Graph delta reverification node {node_key} must receive one new blocker"
                )
        for node in self.node_list:
            declared_incoming = {
                edge for edge in edge_set if edge.blocked_node_key == node.node_key
            }
            expected_incoming = {
                TaskBlockerEdge(
            ...
            if declared_incoming != expected_incoming:
                raise TaskGraphError(
                    f"Delta node {node.node_key} blocker list differs from relation additions"
                )
            for resource in node.resource_list:
                unknown_consumer_set = (
                    set(resource.consumer_node_key_list) - known_key_set
                )
                ...
                if node.node_key in resource.consumer_node_key_list:
                    raise TaskGraphError(
                        f"Delta resource {resource.key} repeats its implicit owner as a consumer"
                    )
        object.__setattr__(
    ...
    def normalized_payload(self) -> dict[str, object]:
        ...
        return {
            ...
            "node_list": [
                item.payload()
                for item in sorted(self.node_list, key=lambda item: item.node_key)
            ],
    ...
    def from_payload(cls, payload: object) -> "TaskGraphDelta":
        ...
        try:
            return cls(
                ...
                reverification_node_key_list=list(
                ...
                blocker_edge_list=[
                    TaskBlockerEdge.from_payload(item)
                    for item in payload["blocker_edge_list"]
                ],
            )
        except (TypeError, ValueError) as error:
            raise TaskGraphError(
                "Task graph delta contains an unsupported enum or field value"
            ) from error
