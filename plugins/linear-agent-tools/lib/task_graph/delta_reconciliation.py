...
def delta_reconciliation_plan_build(
    delta: TaskGraphDelta, remote: RemoteProject
) -> ReconciliationPlan:
    ...
    new_resource_key_set = {
        resource.key for node in delta.node_list for resource in node.resource_list
    }
    ...
    missing_existing_key_set = set(delta.existing_node_key_list) - set(
...
def _project_identity_require(delta: TaskGraphDelta, remote: RemoteProject) -> None:
    ...
    if (
        remote.id != delta.project_id
        or remote.team_id != delta.team_id
        or remote.project_key != delta.project_key
    ):
        raise TaskGraphError(
            "Graph delta destination differs from its approved Project identity"
        )
    if remote.description != project_description_build(
        project_key=delta.project_key, source=delta.source
    ):
        raise TaskGraphError(
...
def _remote_issue_map_get(
    ...
    for issue in issue_list:
        if not issue.node_key or issue.node_key in issue_by_node_key_map:
            raise TaskGraphError(
                "Active Project contains an absent or duplicate provider node key"
            )
...
def _delta_issue_owned_fields_require(
...
def _topology_require(
    delta: TaskGraphDelta, remote_issue_by_node_key_map: dict[str, RemoteIssue]
) -> None:
    ...
    cleanup_key_list = [
        key for key, role in role_by_node_key_map.items() if role is TaskRole.CLEANUP
    ]
...
def _issue_role_get(issue: RemoteIssue) -> TaskRole:
    ...
    if len(role_value_set) != 1:
        raise TaskGraphError(
            f"Existing delta node {issue.node_key} must have exactly one task role label"
        )
