...
class TaskCleanupReconciler:
    ...
    def cleanup(self, request: CleanupRequest) -> CleanupResult:
        ...
        with IssueWorkspaceLock(self._config, request.issue_identifier):
            repository_by_origin_url_map = {
                ...
                for item in request.repository_list
            }
            ...
            for origin_url, state in workspace_state_by_origin_url_map.items():
                if state is not None:
                    repository_by_origin_url_map[origin_url].state_identity_require(
                        request.issue_identifier, state
                    )
                else:
            ...
            for resource in request.resource_list:
                if resource.repository_url not in repository_by_origin_url_map:
                    raise TaskCleanupError(
                        f"Resource {resource.key} has no exact participating repository"
                    )
            closed_pull_request_count = 0
            canceled = (
                request.authority.issue_status == "Canceled"
                or request.authority.project_status == "Canceled"
            )
            for reference in request.pull_request_list:
                before = self._github.inspect(
                ...
                before.target_require(
                    base_branch=target.base_branch, head_branch=target.head_branch
                )
                if canceled:
                    ...
                    raise TaskCleanupError(
                        "Successful task cleanup requires every linked pull request to be merged"
                    )
            ...
            for resource in request.resource_list:
                ...
                working_directory = (
                    task_root if task_root is not None else repository.main_root
                )
                placeholder_by_name_map = (
                    ...
                    if state is not None
                        ...
                        "task_root": str(
                            ...
                            / ".worktree"
                            / request.issue_identifier.lower()
                        ),
                    }
                )
                ...
                if state is not None and request.authority.scope != "attempt":
                    cleaned_resource_fingerprint_by_resource_key_map[resource.key] = (
                        resource.fingerprint()
                    )
                    state = replace(
                        state,
                        cleaned_resource_fingerprint_by_resource_key_map=dict(
                            sorted(
                                cleaned_resource_fingerprint_by_resource_key_map.items()
                            )
                        ),
                    )
            ...
            for origin_url, repository in repository_by_origin_url_map.items():
                ...
                if not state.cleanup_binding_completed:
                    ...
                    cleanup_binding_run(
                        ...
                        placeholder_by_name_map=state.cleanup_placeholder_map(
                            request.issue_identifier
                        ),
                    )
                ...
                if state.worktree_removed and (
                    task_root.exists() or registered_branch is not None
                ):
                    raise TaskCleanupError(
                        "Removed task worktree reappeared after durable cleanup"
                    )
                if (
                    registered_branch is not None
                    and registered_branch != state.branch_name
                ):
                    raise TaskCleanupError(
                        "Task worktree registration changed after durable cleanup snapshot"
                    )
                state = self._cleanup_branch_snapshot_prepare(
                    repository,
                    state,
                ...
                if not state.cleanup_worktree_removal_ready:
                    if request.authority.issue_status != "Canceled":
                        if not task_root.is_dir():
                            raise TaskCleanupError(
                                "Successful task worktree disappeared before dirty-state proof"
                            )
                        ...
                        if dirty:
                            raise TaskCleanupError(
                                "Successful task worktree contains uncommitted user work"
                            )
                    state = replace(state, cleanup_worktree_removal_ready=True)
                    repository.state_write(state)
                if not state.worktree_removed and (
                    task_root.exists() or registered_branch is not None
                ):
                    GitCommand.run(
                        repository.main_root,
                ...
                if state.remote_branch_removed and remote_exists:
                    raise TaskCleanupError(
                        "Removed remote task branch reappeared after durable cleanup"
                    )
                if not state.remote_branch_removed and remote_exists:
                    if not state.cleanup_remote_branch_commit:
                        raise TaskCleanupError(
                            "Remote task branch appeared after the durable cleanup snapshot"
                        )
                ...
                if state.local_branch_removed and local_exists:
                    raise TaskCleanupError(
                        "Removed local task branch reappeared after durable cleanup"
                    )
                if not state.local_branch_removed and local_exists:
                    if (
                        repository.commit_get(state.branch_name)
                        != state.cleanup_local_branch_commit
                    ):
                        raise TaskCleanupError(
                            "Local task branch changed after the durable cleanup snapshot"
                        )
                    GitCommand.run(
                        repository.main_root, ("branch", "-D", state.branch_name)
                    )
                ...
                if (
                    ...
                    raise TaskCleanupError(
                        "Task workspace or branch remained after cleanup"
                    )
    ...
    def _absence_require(
        self, repository: WorkspaceRepository, issue_identifier: str
    ) -> None:
        ...
        if (
            ...
            raise TaskCleanupError(
                "Task resources exist without private ownership proof"
            )
        ...
        *,
        ...
        if state.cleanup_branch_snapshot_ready:
            ...
            if repository.exist_remote_branch(state.branch_name):
                ...
                if (
                    ...
                    raise TaskCleanupError(
                        "Remote task branch changed after the durable cleanup snapshot"
                    )
            return state
        if not repository.exist_local_branch(state.branch_name):
            raise TaskCleanupError(
                "Local task branch disappeared before its durable cleanup snapshot"
            )
        ...
        if successful:
            base_commit = repository.commit_get(
                f"refs/remotes/origin/{state.base_branch}"
            )
    ...
    def _pull_request_contract_require(
        ...
        repository_by_github_identity_map = {
            ...
            if (identity := _github_repository_identity_get(repository.origin_identity))
            is not None
        }
        reference_by_repository_identity_map = {
            item.repository.value: item for item in request.pull_request_list
        }
        for reference in request.pull_request_list:
            if reference.repository.value not in repository_by_github_identity_map:
                raise TaskCleanupError(
                    "Linked pull request is outside the participating GitHub repository set"
                )
        for identity, repository in repository_by_github_identity_map.items():
            ...
            if len(expected_number_list) > 1:
                raise TaskCleanupError(
                    "More than one pull request exists for the exact task branch and base"
                )
            ...
            if expected_number_list != provided_number_list:
                raise TaskCleanupError(
                    "Cleanup request omits or substitutes the exact task pull request"
                )
            if provided is not None:
                snapshot = self._github.inspect(
                    repository=provided.repository, number=provided.number
                )
    ...
    def _pull_request_target_get(
        ...
        *,
        ...
        matching_repository_list = [
            repository
            for repository in repository_list
            if _github_repository_identity_get(repository.origin_identity)
            == github_repository
        ]
        if len(matching_repository_list) != 1:
            raise TaskCleanupError(
                "Linked pull request has no unique participating repository target"
            )
    ...
    def _task_root_require(
        ...
        if (
            repository.main_root != self._config.root
            and repository.main_root.parent != self._config.root
        ):
            raise TaskCleanupError(
                "Owned task worktree is outside the configured workspace"
            )
        task_root = Path(state.task_root)
        if task_root.is_symlink() or not task_root.is_dir():
            raise TaskCleanupError(
                "Owned task worktree is unavailable before project-owned cleanup execution"
            )
        try:
...
def _github_repository_identity_get(origin_identity: str) -> str | None:
    ...
    if len(path_part_list) != 2:
        raise TaskCleanupError(
            "Participating GitHub origin does not identify exact owner/repository"
        )
