"""Typed provider-owned comment envelopes for direct workflow evidence."""
...
class EvidenceCommentCodec:
    ...
    def payload_parse(self, value: str) -> object:
        ...
        if not isinstance(value, str) or not value.startswith(self.prefix) or not value.endswith(_COMMENT_SUFFIX):
            raise EvidenceContractError(f"{self.label} comment has another shape")
        encoded = value[len(self.prefix) : -len(_COMMENT_SUFFIX)]
        try:
            return json_load_strict(encoded)
        except JsonContractError as error:
            raise EvidenceContractError(f"{self.label} comment contains malformed JSON") from error
...
TASK_WORKSPACE_BASELINE_COMMENT_CODEC = EvidenceCommentCodec(
