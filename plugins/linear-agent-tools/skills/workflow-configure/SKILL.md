...
Every preview or handoff response MUST distinguish the fully paginated team issue statuses, workspace Project statuses, labels and Git status automations. State explicitly that the no-echo in-memory GraphQL transaction deletes only exact approved automation rules and creates only still-missing approved status definitions before any other global mutation, and that supported label creation belongs to official Linear MCP operations performed afterward. Final success requires an exact destination read-back with no remaining delta and an empty Git status automation list.
