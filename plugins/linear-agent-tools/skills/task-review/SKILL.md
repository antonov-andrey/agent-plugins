...
If the candidate changes `linear-agent-tools:task-review` or the provider lifecycle that would launch or govern this review, use a fresh generic max-reasoning thread instead of activating the installed/cached provider under review. Give that thread one complete prompt and the branch-local paths to this contract, the manual, root design and exact base-to-head diff. The branch-local contracts are review authority; the implementation thread and installed plugin cache are not. Do not create a nested agent merely to repeat the same review scope.
...
Before recovering any interrupted or rework path, run attempt cleanup under the live guard. Derive full semantic coverage independently from every applicable owner. A finding must identify a concrete semantic defect or root canonicalization/ownership correction. Consolidate duplicate manifestations into one root-oriented finding and report the complete current justified set in one pass. Do not propose proof-only checks, wrappers, metadata, fingerprints or compatibility branches unless they protect a named semantic boundary.

For code delivery, compare the implementation handoff's complete ordered composite PR candidate list with freshly read current GitHub state. Each candidate contains its URL, base branch, base commit and head commit. Evidence delivery omits inapplicable PR candidates. The handoff is context, not an approval fingerprint. If a declared code candidate value changed, first run attempt cleanup, then publish the stale-state finding and validate `Review -> Rework` with changed reviewed identity. Reuse direct verification only when source, exact command, environment/release and semantic contract are unchanged. Run new checks when that semantic proof is absent.

- Zero findings: for code delivery, first run `../task-merge/scripts/git_transport.py inspect` on the exact merge host and treat any missing or invalid owned runtime as a launchability finding; review never provisions it. Run attempt cleanup, then publish a minimal handoff, fully paginate comments, parse the provider-marked result and semantically match every field consumed by the transition. Its human summary states the zero-finding result. Include exact composite PR candidates for code delivery and omit them for evidence delivery. Include direct check results with applicable evidence links. Validate current reviewed identity when code applies. Move code delivery `Review -> Merging`; move evidence implementation `Review -> Done`.
...
For both modes, attempt cleanup is a mandatory pre-handoff and pre-transition step for success, finding, stale-state, failed, canceled and controlled interrupted outcomes. No status mutation or handoff publication may precede it. An abrupt process loss publishes nothing; the next guarded attempt repeats idempotent cleanup before recovery. Use native background-terminal waiting for external checks, never model polling/supervisors/timeouts/thresholds or alternate Codex homes. If a harness launches a child, use standard `HOME`, unset `CODEX_HOME`, provide the complete prompt and close stdin in one launch operation, and do not forward Linear credentials. Handoffs carry any nonempty subset of exact known structured Codex usage counters only when directly exposed; omit usage when none are exposed and never estimate. No verification receipt, candidate fingerprint or generic invalidation gate is created.
