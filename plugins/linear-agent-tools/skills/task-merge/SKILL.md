...
Every preview or handoff response MUST state that one exact issue process-lifetime host-local attempt guard is acquired before dispatch or status mutation, held continuously across ordered merge recovery, nested attempt cleanup and the final Linear provider readback, and released only by process exit after that boundary.
