...
- `workflow-infrastructure-development-environment` owns `project_id`, `owner_issue_identifier`, the normalized `workflow-infrastructure` repository and one dated `common_prefix`. Terminal-issue retention readback completes before owner-worktree retirement, requires that exact clean published worktree and invokes its committed `destroy-inventory` boundary. Destructive Project-final cleanup occurs after every task-worktree retirement, so it guarded-fast-forwards clean canonical `main` and invokes the merged `destroy` boundary there. Both paths call only that Product's fixed `development_environment_manage.py <operation> --git-worktree <common-prefix>` boundary with typed JSON stdin and require the invoked ordinary file to match the selected commit.
...
The repository-private file retains only the first-attempt baseline needed to prove branch ancestry. Deterministic state/bootstrap temporary paths live only below that issue's private Git-admin owner; recovery removes only exact manifest-derived stale paths and preserves foreign siblings before workspace retirement. The provider derives current worktree/branch identities from the issue, rereads live Git/GitHub state before each destructive step, removes the exact registered worktree, remote branch with an exact lease, local branch and then the private baseline. Before every Git process, the same complete minimal standard-user invocation owner directly reads physical repository config, strictly parses every origin and `.gitmodules` URL, resolves an allowed relative submodule URL against its already validated parent, rejects credentials, local/file and helper forms plus ambient includes, rewrites and `protocol.*`, then enables only the parsed exact GitHub HTTPS/SSH protocol over `protocol.allow=never`. It ignores global/system and invocation Git config, disables replacement and alternate object resolution, disables all hooks with `core.hooksPath=/dev/null`, and adds `--no-verify` to push. Remote deletion resolves one exact fetch URL and one exact push URL, canonicalizes both to the same approved repository, directly reads the current head from that push target and passes that explicit target to the leased push; multiple, unknown or divergent destinations stop before remote mutation. Current provider state is crash recovery: retry continues from the first target still present and creates no durable phase, resource snapshot or cleanup journal.
