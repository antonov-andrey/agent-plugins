...
- `workflow-infrastructure-development-environment` owns `project_id`, `owner_issue_identifier`, the normalized `workflow-infrastructure` repository and one dated `common_prefix`. It synchronizes a clean canonical `main` checkout by guarded fast-forward and calls only that Product's fixed `development_environment_manage.py destroy-inventory|destroy --git-worktree <common-prefix>` boundary with typed JSON stdin.
