...
- Apply the complete shared [Verification Receipt Contract](../../references/manual-workflow.md#verification-receipt-contract) to every verification result. Create each receipt through `../../lib/verification/tool/evidence.py receipt-create` at the exact result-completion instant, publish its codec-rendered comment, prove one byte-identical authenticated provider readback, and run `receipt-reuse` against that exact readback before candidate publication. Preserve every output and every externally stored consumed file as independently readable immutable provider artifacts and include their contract-owned identities. A source-independent probe may use an empty checkout list, but its source fingerprint remains required.
...
Every receipt-bearing preview or handoff must cite the shared contract semantically and report the concrete evidence produced in this attempt: receipt keys, immutable input and output artifact identities, exact provider comment creation/readback results, reuse decisions, and the candidate identity/fingerprint. Do not satisfy this requirement with prescribed phrases or an inputs-only summary; the evidence must demonstrate that the shared owner actually created, read back and reused each receipt.

Before publishing the attempt result, invoke `task-cleanup` for all current `attempt`-lifetime resources. Build the exact code or evidence candidate identity and fingerprint with `../../lib/verification/tool/evidence.py candidate`. Evidence delivery supplies a map of evidence kind to complete passed current-schema verification receipt after exact provider readback; the tool validates the derived receipt key and emits the compact map of evidence kind to receipt key that Human Review approves. Never substitute the stable verification key. Build the concise structured attempt comment with its `attempt` operation: attempt ID, role, delivery kind, UTC start/end, outcome, delivery-applicable commit set, receipt hit/miss counts, external wait duration, the exact structured `input_tokens`, `cached_input_tokens`, `cache_write_input_tokens`, `output_tokens` and `reasoning_output_tokens` counters only when directly exposed and aggregated independently across every nested invocation, complete compact candidate identity, its derived fingerprint, PR/CI links and bounded evidence. Omit unavailable usage and never substitute estimates, log-derived values or a derived total. Publish and reread that exact comment in Linear and require the fingerprint to reproduce from the persisted identity. Exclude prompts, secrets and raw logs; delete transient input files only after this readback.
