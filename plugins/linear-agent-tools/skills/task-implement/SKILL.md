...
- Build receipts from the exact source fingerprint, direct argv, canonical absolute working directory and every result-affecting checkout. Each checkout binds its canonical absolute path, roles, repository URL, full commit, recursive submodules and dependency locks without repository-URL collapsing. Submodule and lock keys are canonical paths relative to that checkout. Never compress this checkout list into a repository commit set: every preview or handoff that discusses a receipt MUST state that path, roles, repository URL, full commit, recursive-submodule identities and dependency-lock identities are bound per checkout. Bind corpus content, model identity, model configuration and applicable environment or release identity. Preserve the exact machine-readable result as one independently readable immutable provider artifact. Bind that artifact's exact URL and SHA-256 through `scripts/receipt.py`. A source-independent evidence probe may use an empty checkout list, but never an absent source fingerprint. Reuse only an exact passed current-schema receipt whose bound artifact remains authoritative.
